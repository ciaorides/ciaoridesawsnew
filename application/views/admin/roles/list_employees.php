            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
                <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">
                      ...
                      <option value="3">Employee Email</option>
                      <option value="4">Employee Mobile</option>
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/roles/all_employees'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            { "title": "Employee ID", "name":"title","orderable": false, "data":"user_id", "width":"0%" },
           
            { "title": "Employee Name", "name":"title","orderable": false, "data":"user_name", "width":"0%" },

             { "title": "Employee Email", "name":"title","orderable": false, "data":"user_email", "width":"0%" },

              { "title": "Employee Mobile", "name":"title","orderable": false, "data":"user_mobile", "width":"0%" },

              { "title": "Roles", "name":"title","orderable": false, "data":"roles", "width":"0%" },
           
           // { "title": "View", "name":"created_on","orderable": false, "data":"id", "width":"0%" },
        ...
        {           
          ...
          var image = '<img src="'+url+aData['image']+'" height="100" width="150">';
          //$(nRow).find('td:eq(2)').html(image);

          var view ='<a  title="view" href="'+url+'admin/roles/view_employee/'+aData['id']+'" class="btn btn-primary btn-condensed">View</a>';
          //$(nRow).find('td:eq(6)').html(view);

          if(aData['status'] == "Active")
          {
            var action = '<a title="Click to Inactive" href="'+url+'admin/roles/change_employee_status/'+aData['id']+'/Inactive" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var action = '<a title="Click to Active" href="'+url+'admin/roles/change_employee_status/'+aData['id']+'/Active" class="btn btn-danger btn-condensed">Inactive</a>';
          }
        ...
        <?php }?>
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/roles/edit_employees/'+aData['id']+'"><i class="fa fa-edit"></i></a>';
         
      
      <?php if( (in_array('s',$roleResponsible['employees'])) && (in_array('e',$roleResponsible['employees'])) ){?>
        
          $(nRow).find('td:eq(8)').html(action);
        
        <?php }else if((!in_array('s',$roleResponsible['employees'])) && (in_array('e',$roleResponsible['employees'])) ){?>
           $(nRow).find('td:eq(7)').html(action);
